...
def perform_hourly_prediction(ticker, start_date, end_date):
    ...
    jsonDataHourFrame = {
        ...
        "predicted_close2": [],
        "predicted_close3": [],
        "matches": [],  # Add matches to the JSON structure
    }
    ...
    # Identify matches within 5% of the actual open value
    for candlestick in jsonDataHourFrame["candlestick"]:
        actual_open = candlestick["open"]
        time_unix = candlestick["time"]

        for i in range(1, 4):
            predicted_open = jsonDataHourFrame[f"predicted_open{i}"]
            predicted_open_value = next(
                (item["value"] for item in predicted_open if item["time"] == time_unix), None
            )
            if predicted_open_value is not None:
                difference = abs((actual_open - predicted_open_value) / actual_open) * 100
                if difference <= 5:
                    jsonDataHourFrame["matches"].append({"time": time_unix, "value": actual_open})

    return jsonDataHourFrame

# Streamlit UI for user inputs
st.title("Hourly OHLC Prediction")
ticker = st.text_input("Enter stock ticker:")
start_date = st.date_input("Enter start date:")
end_date = st.date_input("Enter end date:")

if st.button("Predict and Display Charts"):
    # Perform hourly prediction and get JSON data
    jsonDataHourFrame = perform_hourly_prediction(ticker, start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"))

    # Checkbox options for displaying predicted values and matches
    show_predicted_open1 = st.checkbox("Show Predicted Open1")
    show_predicted_open2 = st.checkbox("Show Predicted Open2")
    show_predicted_open3 = st.checkbox("Show Predicted Open3")
    show_matches = st.checkbox("Show Matches within 5% of Actual Open")

    # Convert candlestick and predicted data to DataFrame
    candlestick_df = pd.DataFrame(jsonDataHourFrame["candlestick"])
    predicted_open1_df = pd.DataFrame(jsonDataHourFrame["predicted_open1"])
    predicted_open2_df = pd.DataFrame(jsonDataHourFrame["predicted_open2"])
    predicted_open3_df = pd.DataFrame(jsonDataHourFrame["predicted_open3"])
    matches_df = pd.DataFrame(jsonDataHourFrame["matches"])

    # Render candlestick chart
    st.subheader(f"Candlestick Chart for {ticker}")
    st.line_chart(candlestick_df.set_index("time")[["open", "high", "low", "close"]])

    # Optionally render predicted values and matches
    if show_predicted_open1:
        st.subheader("Predicted Open1")
        st.line_chart(predicted_open1_df.set_index("time")["value"])

    if show_predicted_open2:
        st.subheader("Predicted Open2")
        st.line_chart(predicted_open2_df.set_index("time")["value"])

    if show_predicted_open3:
        st.subheader("Predicted Open3")
        st.line_chart(predicted_open3_df.set_index("time")["value"])

    if show_matches:
        st.subheader("Matches within 5% of Actual Open")
        st.line_chart(matches_df.set_index("time")["value"])
